...
# Importa scripts do mesmo diretório
...
def random_date_past_range(min_years: int, max_years: int) -> date:
    """Data aleatória entre min_years e max_years atrás."""
...
def assign_profiles(nomes: list) -> dict:
    """
    ...
    for i, nome in enumerate(nomes):
        ...
        elif i < n_critico + n_alto:
            profiles[nome] = "alto"
        elif i < n_critico + n_alto + n_medio:
            profiles[nome] = "medio"
        else:
            profiles[nome] = "bom"

    cnt = Counter(profiles.values())
    log.info(
        "  Perfis: Crítico=%d (%.0f%%), Alto=%d (%.0f%%), Médio=%d (%.0f%%), Bom=%d (%.0f%%)",
        cnt["critico"], cnt["critico"] / n * 100,
        cnt["alto"],    cnt["alto"]    / n * 100,
        cnt["medio"],   cnt["medio"]   / n * 100,
        cnt["bom"],     cnt["bom"]     / n * 100,
    )
    return profiles
...
def generate_rede_mt(municipios_gdf: gpd.GeoDataFrame, profiles: dict) -> gpd.GeoDataFrame:
    ...
    for _, row in municipios_gdf.iterrows():
        ...
        for i in range(n_segs):
            ...
            # Vary installation date based on risk profile
            if perfil == "critico":
                data = random_date_past_range(25, 40)
            elif perfil == "alto":
                data = random_date_past_range(15, 30)
            elif perfil == "medio":
                data = random_date_past_range(8, 25)
            else:  # bom
                data = random_date_past_range(2, 15)
            rows.append({
...
def generate_transformadores(municipios_gdf: gpd.GeoDataFrame, profiles: dict) -> gpd.GeoDataFrame:
    ...
    for _, row in municipios_gdf.iterrows():
        ...
        for i in range(n_trans):
            ...
            if perfil == "critico":
                data = random_date_past_range(20, 35)
            elif perfil == "bom":
                data = random_date_past_range(2, 15)
            else:
                data = random_date_past(30)
            rows.append({
...
# ---------------------------------------------------------------------------
# Geração de religadores
# ---------------------------------------------------------------------------

def generate_religadores(municipios_gdf: gpd.GeoDataFrame, profiles: dict) -> gpd.GeoDataFrame:
    """Gera religadores sintéticos (menos para municípios críticos)."""
    log.info("Gerando religadores...")
    rows = []
    for _, row in municipios_gdf.iterrows():
        nome = row["nome"]
        perfil = profiles.get(nome, "medio")
        # Critical municipalities have fewer (or no) reclosers — worse protection
        if perfil == "critico":
        ...
        for i in range(n_rel):
            pt = random_point_in_polygon(row["geom"])
            rows.append({
                "cod_id": f"RE-DEMO-{row['codigo_ibge']}-{i:03d}",
                "distribuidora": DISTRIBUIDORA,
                "municipio": nome,
...
# ---------------------------------------------------------------------------
# Geração de chaves seccionadoras
# ---------------------------------------------------------------------------

def generate_chaves(municipios_gdf: gpd.GeoDataFrame, n_rel_per_mun: dict) -> gpd.GeoDataFrame:
    """Gera chaves seccionadoras sintéticas (aprox. n_religadores / 2 por município)."""
    ...
    for _, row in municipios_gdf.iterrows():
        ...
        for i in range(n_chaves):
            pt = random_point_in_polygon(row["geom"])
            rows.append({
                "cod_id": f"CH-DEMO-{row['codigo_ibge']}-{i:03d}",
                "distribuidora": DISTRIBUIDORA,
                "municipio": nome,
                "uf": row["uf"],
...
def generate_dec_fec(municipios_gdf: gpd.GeoDataFrame, profiles: dict) -> pd.DataFrame:
    """
    Gera indicadores DEC/FEC mensais com 4 perfis de risco bem diferenciados.

    Perfis:
      critico: DEC base 28h (limite 12h) → score esperado 75-95
      alto:    DEC base 18h (limite 12h) → score esperado 55-74
      medio:   DEC base 13h (limite 12h) → score esperado 35-54
      bom:     DEC base  6h (limite 12h) → score esperado 10-34
    """
...
def clear_existing(engine, uf: str) -> None:
    ...
    with engine.begin() as conn:
        tables = [
            ("mapa_risco",              "DELETE FROM mapa_risco WHERE uf = :uf"),
            ("historico_score",          "DELETE FROM historico_score WHERE uf = :uf"),
            ("gaps_protecao",            "DELETE FROM gaps_protecao WHERE uf = :uf"),
            ("indicadores_continuidade", "DELETE FROM indicadores_continuidade WHERE uf = :uf"),
            ("chaves",                   "DELETE FROM chaves WHERE uf = :uf"),
            ("religadores",              """DELETE FROM religadores WHERE distribuidora = :dist
                                            AND municipio IN (SELECT nome FROM ibge_municipios WHERE uf = :uf)"""),
            ("rede_mt",                  "DELETE FROM rede_mt WHERE uf = :uf"),
            ("transformadores",          """DELETE FROM transformadores WHERE distribuidora = :dist
                                            AND municipio IN (SELECT nome FROM ibge_municipios WHERE uf = :uf)"""),
        ]
        for table, sql in tables:
            deleted = conn.execute(text(sql), {"uf": uf, "dist": DISTRIBUIDORA}).rowcount
            if deleted:
...
def main(uf: str, limpar: bool, db_url: Optional[str]) -> None:
    ...
    # Gera perfis de risco antes de qualquer outra geração
    ...
    rede_gdf = generate_rede_mt(municipios_gdf, profiles)
    insert_rede_mt(rede_gdf, engine)

    # ── 3. Transformadores ────────────────────────────────────────────────────
    log.info("[3/9] Gerando transformadores sintéticos...")
    trans_gdf = generate_transformadores(municipios_gdf, profiles)
    ...
    rel_gdf = generate_religadores(municipios_gdf, profiles)
    insert_religadores(rel_gdf, engine)

    n_rel_per_mun: dict = {nome: 0 for nome in municipios_gdf["nome"].tolist()}
    for _, row in rel_gdf.iterrows():
        n_rel_per_mun[row["municipio"]] = n_rel_per_mun.get(row["municipio"], 0) + 1

    chaves_gdf = generate_chaves(municipios_gdf, n_rel_per_mun)
