...
Downloads municipality population estimates from the IBGE API and inserts
...
IBGE_POP_URL = "https://servicodados.ibge.gov.br/api/v3/agregados/6579/periodos/2022/variaveis/9324?localidades=N6[all]"
...
def ingest_uf(uf: str, engine) -> int:
    ...
    log.info("[%s] Fetching population estimates (2022 census)...", uf)
    pop_data = fetch_json(IBGE_POP_URL)

    # Build dict: code -> population
    ...
    try:
        for item in pop_data[0]["resultados"][0]["series"]:
            ...
            if valor and valor != "...":
                try:
                    pop_map[codigo] = int(valor)
                except (ValueError, TypeError):
                    pass
    except (KeyError, IndexError, TypeError) as exc:
        log.warning("Could not parse population data: %s", exc)
